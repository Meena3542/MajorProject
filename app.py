...
# Sidebar content (e.g., input fields, text, etc.)
st.sidebar.markdown("Use the form below to input student data.")
st.sidebar.image("https://img.freepik.com/premium-vector/back-school-illustration_1302918-34036.jpg?semt=ais_hybrid")
st.sidebar.markdown("<a href='http://dropoutmanagementsystem.42web.io/login/'>Click Here to Login!</a>", unsafe_allow_html=True)
...
# Function to update session state globally
...
if st.button("Predict Dropout"):
    ...
    # Academic low credits check
    low_credits_threshold = 9  # Define a threshold for low credits
    credited_units = input_df['Curricular_units_1st_sem_credited'][0]
    is_low_credits = credited_units < low_credits_threshold
    # Display dropout and suggestions
    if dropout_status == 'Dropout':
        ...
        if debtor_status == 'Yes':
            ...
            st.markdown("[Click here to explore the NEP Education Policy](https://ignou-nep-pdp.samarth.ac.in/index.php/registration/user/register)")
        if is_low_credits:
            st.warning("The student has low academic credits. Encourage them to upskill.")
            st.markdown("[Click here to explore upskilling opportunities](https://eduskills.academy/#categories)")
    else:
        ...
        if debtor_status == 'Yes':
            ...
            st.markdown("[Click here to explore the NEP Education Policy](https://ignou-nep-pdp.samarth.ac.in/index.php/registration/user/register)")
        if is_low_credits:
            st.warning("The student has low academic credits. Encourage them to improve their performance.")
            st.markdown("[Click here to explore upskilling opportunities](https://eduskills.academy/#categories)")

    # If neither condition is met
    if not is_low_credits and debtor_status == 'No':
        st.info("The student shows no signs of financial or academic issues.")
    #if status_map[prediction[0]]=='Dropout':
        #st.markdown("<h3>Recommended actions to be taken for dropout issue:</h3><ul><li>Enhance Support for Academic Performance</li><li>Ensure Timely Payment of Tuition Fees</li><li>Increase Monitoring for Debtor Students</li><li>Expand Scholarship Programs<ul>",unsafe_allow_html=True)
